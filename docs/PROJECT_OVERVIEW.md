...
**K-IFRS 제1115호(고객과의 계약에서 생기는 수익)** 에 관하여, **회계감사인(감사인)**들이 객관적인 팩트를 손쉽게 찾고, AI의 구조화된 가이드를 통해 판단을 내릴 수 있도록 하는 **전문 도구**이다.
...
이 프로젝트는 **2종오류 방지에 집중**한다. LLM이 자유롭게 추론하면 그럴듯하지만 틀린 답(환각)을 생성할 수 있고, 이는 곧 실재하는 회계 쟁점을 놓치는 2종오류로 이어진다.
...
> 이 설계는 **규칙 기반 전문가 시스템**에 가깝다.
> 새로운 사업모델(메타버스, DeFi), 타 기준서 교차(1116호 리스 + 1115호 수익), 고도의 복합적 사실판단(Gray Area)에서는 효과가 제한된다.
> 대신, K-IFRS 1115호 범위 내에서 **기존 판례와 기준서 원문에 근거한 답변의 신뢰성**을 극대화한다.
...
이 프로젝트의 핵심 차별점은 **5단계 방어 레이어**로 LLM 환각을 구조적으로 차단하는 것이다.
...
`analyze` 노드가 추출한 키워드를 `MASTER_DECISION_TREES`의 `trigger_keywords`와 매칭하여, `is_situation=True`일 때 `clarify_agent`의 system prompt에 체크리스트 + 결론가이드를 동적 주입한다.
